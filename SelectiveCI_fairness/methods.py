...
class FaReG(nn.Module):
    def __init__(self, num_features, delta, device, hidden_dim: int = 64, latent_dim: int = 32):
        super().__init__()
        self.device = device
        self.delta = delta
        self.v = -1.0
...
class FaReG_Rec(nn.Module):
    def __init__(self, num_features, device, hidden_dim: int = 64, latent_dim: int = 32):
        super().__init__()
        self.device = device
        self.decoder = nn.Sequential(
            nn.Linear(4, latent_dim),

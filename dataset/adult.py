...
def build_dataloaders_adult(cfg: SimpleNamespace):
    ...
    # attr: sex 
    sex = (df["sex"] == "Female").astype(np.int32).to_numpy()
    ...
    x, y, color, age_bucket, sex = x[idx], y[idx], color[idx], age_bucket[idx], sex[idx]

    n_total = len(df)
    n_use = int(getattr(cfg, "n_use", 0) or 0)
    if n_use > 0 and n_use < n_total:
        n_total = n_use
        x, y, color, age_bucket, sex = x[:n_total], y[:n_total], color[:n_total], age_bucket[:n_total], sex[:n_total]
    ...
    def make_loader(sl: slice, shuffle: bool):
        ...
        rb = torch.tensor(sex[sl], dtype=torch.long)
